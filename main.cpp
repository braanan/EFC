...
int main()
{
    ...
    std::string line;
    std::ifstream infile;  infile.open( "/Users/benya/Documents/Xcode/EFC/seed/grab.txt" );
    std::ofstream outfile; outfile.open( "/Users/benya/Documents/Xcode/EFC/seed/EFC_output.txt" );
    ...
    while( std::getline( infile, line ) )
    {
        if( ++i > 1 ) // skip header line
        {
            ...
            double matime, depth, depthRate, speedCmd, pitchAngle, elevAngle, verticalMode;
            in >> matime >> depth >> depthRate >> speedCmd >> pitchAngle >> elevAngle >> verticalMode;
            
            
            //check incoming data and make sure operational profile is compatible for analysis
            bool  dataCheckEFC[7] = { false }; int dataChecksumEFC = 0;
            ...
            if( !isnan( verticalMode ) )        { dataCheckEFC [5] = true; }
            if( depth > 2 && speedCmd > 0.8 && verticalMode==5)   { dataCheckEFC [6] = true; }
            ...
            if( dataChecksumEFC == 7 )
            {
                ...
                if( enableElevOffsetCalc_ == 1)
                {
                    
                    //pulse counter
                    empiricalFaultCounter_ += 1;
                    
                    //sum-up elev angle for bin avg.
                    empiricalFaultEleTmp_ += elevAngle;
                    
                    if(empiricalFaultCounter_ == 7200)
                    {
                        
                        ElevOffset = empiricalFaultEleTmp_ / 7200;  //calc mean elevator angle (offset)
                        empiricalFaultCounter_ = 0;                 //reset counter
                        enableElevOffsetCalc_ = 0;                  //disable Elev offset calculator
                        
                        //write to screen
                        std::cout << "\n";
                        std::cout << "ElevOffset = " << ElevOffset * (180/pi) << "\n";
                    }
                }
                
                
                if( enableElevOffsetCalc_ == 0)
                {
                    //sum-up for bin avg.
                    ...
                    pitchAngle = pitchAngle - ElevOffset; //correct offset
